...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    else if (target_type_ == TARGET_TYPE::ROS_TUTORIAL)
      tutorial_wp_sub_ = nh.subscribe("/waypoint_generator/tutorial_goal", 1, &EGOReplanFSM::tutorialWaypointCallback, this);
  ...
  {
    Eigen::Vector3d goal;
    goal <<  msg->poses[0].pose.position.x, msg->poses[0].pose.position.y, msg->poses[0].pose.position.z;
    setGoal(goal);
  }

  void EGOReplanFSM::tutorialWaypointCallback(const ego_planner::TutorialGoal::ConstPtr &msg)
  {
    Eigen::Vector3d goal;
    goal <<  msg->goal_x, msg->goal_y, msg->goal_z;
    setGoal(goal);
  }

  void EGOReplanFSM::setGoal(const Eigen::Vector3d& goal){
