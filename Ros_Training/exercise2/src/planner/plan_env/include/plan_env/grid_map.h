...
public:
  GridMap() {
    ros::NodeHandle grid_map_nh_;
    service_ = grid_map_nh_.advertiseService("/call_valid", &GridMap::CallValid, this);
    check_valid_thread_= std::thread(std::bind(&GridMap::CheckThread,this));
  }
  ~GridMap() {
    CheckValidStop();
  }
  ...
  void CheckValidStop() {
    if (check_valid_thread_.joinable()) {
      check_valid_thread_.join();
    }
  }
  ...
  std::thread check_valid_thread_;

  enum { POSE_STAMPED = 1, ODOMETRY = 2, INVALID_IDX = -10000 };

  bool CallValid(plan_env::lec4::Request  &req,
                 plan_env::lec4::Response &res) {
  Eigen::Vector3d call_point;
  call_point << req.x, req.y, req.z;
  std::cout << call_point << std::endl;
  int valid_flag = getInflateOccupancy(call_point);
  ROS_WARN("call point %f %f %f", req.x, req.y, req.z);
  ROS_WARN("valid: %d", valid_flag);
  if (valid_flag) {
    res.is_valid = false;
  } else {
    res.is_valid = true;
